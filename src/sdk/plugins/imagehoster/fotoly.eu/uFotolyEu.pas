...
type
  TFotolyEu = class(TImageHosterPlugIn)
  protected
  const
    WEBSITE = 'https://fotoly.eu/';
...
function TFotolyEu.HasAPIKey: Boolean;
begin
  Result := Trim(AccountName) <> '';
  if not Result then
    ErrorMsg := 'Fotoly API key missing. Store the API key in the account name field.';
end;

function TFotolyEu.CreateAPIRequest(const APath: string): IHTTPRequest;
begin
...
function TFotolyEu.ExecuteRequest(const AHTTPRequest: IHTTPRequest; const AHTTPParams: IHTTPParams; out AHTTPProcess: IHTTPProcess): Boolean;
var
  LRequestID: Double;
begin
  AHTTPProcess := nil;
  ...
  if not Result then
    ErrorMsg := 'Fotoly request returned no HTTP process result.';
end;

function TFotolyEu.ExecuteJSONRequest(const AHTTPRequest: IHTTPRequest; const AHTTPParams: IHTTPParams; out AJSON: TlkJSONobject): Boolean;
var
...
begin
  ...
  try
    ...
    if not Result then
      ErrorMsg := 'Fotoly returned a non-object JSON response.';
  except
    on E: Exception do
    begin
      ErrorMsg := 'Fotoly returned invalid JSON: ' + E.Message;
    end;
  end;
end;

function TFotolyEu.GetAPIErrorMessage(const ASourceCode, AFallback: string): string;
var
  LJSON: TlkJSONobject;
  LMessage: string;
  LDetails: string;
...
begin
  ...
  if Result = '' then
    Result := 'Fotoly request failed.';
end;

function TFotolyEu.TryGetObjectField(const AParent: TlkJSONbase; const AName: WideString; out AValue: TlkJSONobject): Boolean;
var
  LField: TlkJSONbase;
begin
  AValue := nil;
...
function TFotolyEu.TryGetArrayField(const AParent: TlkJSONbase; const AName: WideString; out AValue: TlkJSONlist): Boolean;
var
  LField: TlkJSONbase;
begin
  AValue := nil;
...
function TFotolyEu.TryGetStringField(const AParent: TlkJSONbase; const AName: WideString; out AValue: string): Boolean;
var
  LField: TlkJSONbase;
begin
  AValue := '';
...
function TFotolyEu.TryGetBooleanField(const AParent: TlkJSONbase; const AName: WideString; out AValue: Boolean): Boolean;
var
  LField: TlkJSONbase;
  LValue: string;
begin
...
function TFotolyEu.IsAllowedFormat(const AAllowedFormats: TlkJSONlist; const AFormat: string): Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TFotolyEu.AddDerivative(const ADerivatives: TlkJSONlist; const AFamily, ASize: string);
var
...
function TFotolyEu.GetLocalFileSize(const ALocalPath: WideString): Int64;
var
...
function TFotolyEu.LoadCapabilities(out AAllowOriginal, AAllowWebp, AAllowAvif: Boolean): Boolean;
var
...
function TFotolyEu.CreateUploadSession(const ALocalPath: WideString; AAllowOriginal, AAllowWebp, AAllowAvif: Boolean; out AUploadURL, AUploadToken: string): Boolean;
var
...
begin
  ...
  try
    ...
    if not Result then
      ErrorMsg := 'Fotoly upload session response is missing upload_url or token.';
  finally
    LResponseJSON.Free;
  end;
end;

function TFotolyEu.UploadFile(const AUploadURL, AUploadToken: string; const ALocalPath: WideString; out AImageUUID: string; out AUploadResponse: TlkJSONobject): Boolean;
var
  LRequest: IHTTPRequest;
  LHTTPParams: IHTTPParams;
begin
  ...
  if not Result then
  begin
    ErrorMsg := 'Fotoly upload response is missing image_uuid.';
    FreeAndNil(AUploadResponse);
  end;
end;

function TFotolyEu.WaitForProcessing(const AImageUUID: string; out AComplete: Boolean): Boolean;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    try
      ...
      if LFailed then
      begin
        ErrorMsg := 'Fotoly image processing failed.';
        Result := False;
        Exit;
      end;
...
function TFotolyEu.LoadImageResource(const AImageUUID: string; out AImageJSON: TlkJSONobject): Boolean;
begin
  Result := ExecuteJSONRequest(CreateAPIRequest('images/' + AImageUUID), nil, AImageJSON);
end;

function TFotolyEu.TryGetVariantURL(const AJSON: TlkJSONobject; const ARootName, AFamily, ASize: string; ARequireReady: Boolean; out AURL: string): Boolean;
var
...
function TFotolyEu.SelectPreferredURL(const AJSON: TlkJSONobject): string;
begin
  Result := '';
...
function TFotolyEu.ExtractURLExtension(const AURL: WideString): string;
var
  LValue: string;
  LQueryPos: Integer;
  LHashPos: Integer;
...
function TFotolyEu.DownloadRemoteFile(const ARemoteURL: WideString; out ATempFile: string): Boolean;
var
...
begin
  ...
  begin
    ErrorMsg := 'Could not resolve a temporary directory for Fotoly remote upload.';
    Exit;
  end;

  if Windows.GetTempFileName(LTempPath, 'fty', 0, LTempFileName) = 0 then
  begin
    ErrorMsg := 'Could not create a temporary file for Fotoly remote upload.';
    Exit;
  end;
  ...
  begin
    ErrorMsg := 'Fotoly remote upload could not download the source URL first.';
...
function TFotolyEu.GetName: WideString;
begin
  Result := 'Fotoly.eu';
end;

function TFotolyEu.LocalUpload(const ALocalPath: WideString; out AUrl: WideString): WordBool;
var
...
begin
  ...
  try
    ...
    Result := AUrl <> '';
    if not Result then
      ErrorMsg := 'Fotoly did not return a usable image URL.';
  finally
    LUploadResponse.Free;
  end;
end;

function TFotolyEu.RemoteUpload(const ARemoteUrl: WideString; out AUrl: WideString): WordBool;
var
  LTempFile: string;
begin
  Result := False;
